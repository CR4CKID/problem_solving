...
# 적
enemy = pygame.image.load("C:\\Users\\CRACKID\\\
Desktop\\PythonWorkspace\\pygame_basic\\enemy.png")
...
# 시작 시간 정보
start_ticks = time.get_ticks() # 시작 tick을 받아옴

# 이벤트 루프
running = True # 게임이 진행중인가?
while running:

    dt = clock.tick(120) # 게임 화면의 초당 프레임 수
    ...
    for event in pygame.event.get(): # 어떤 이벤트가 발생하였는가?
        ...
        if event.type == pygame.KEYUP: # 방향키를 뗄 경우
    ...
    character_y_pos += to_y * dt # dt 값을 곱해줌으로써 낮음 프레임에서도 게임속도는 변화X
    ...
    character_rect.top = character_y_pos # character_rect 정보 업데이트
    ...
    # 충돌 체크
    if character_rect.colliderect(enemy_rect): # colliderect을 통해 충돌 시를 알 수 있음
        print("충돌했어요")
        running = False
